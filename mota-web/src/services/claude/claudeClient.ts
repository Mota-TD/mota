...
export class ClaudeClient {
  ...
  constructor(config: Partial<ClaudeConfig> = {}) {
    ...
    if (!this.config.apiKey) {
      console.warn('Claude API Key未设置，AI功能将使用模拟数据')
    }
  }
  ...
  async sendMessage(
    ...
    if (!this.config.apiKey) {
      throw new Error('Claude API Key未设置')
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Claude API错误: ${response.status} ${response.statusText} - ${JSON.stringify(errorData)}`)
      }
...
请分析项目风险。`
  ...
  async checkConnection(): Promise<boolean> {
    ...
    try {
      ...
      ], 'Please respond with "OK" if you received this message.')
      return true
    } catch {
      return false
    }
