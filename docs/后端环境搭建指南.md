...
### 0. Docker 镜像拉取失败（网络问题）

**问题**：执行 `docker-compose up -d` 时提示无法连接到 Docker Hub，错误信息类似：
```
...
5. **验证配置**
   ```bash
   docker info | findstr -i "Registry Mirrors" -A 5
   ```
