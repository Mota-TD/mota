...
public interface MilestoneTaskMapper extends BaseMapper<MilestoneTask> {
     ...
     * 更新任务进度
     */
    @Update("UPDATE milestone_task SET progress = #{progress}, updated_at = NOW() WHERE id = #{taskId}")
    ...
    @Select("SELECT mt.*, " +
            ...
            "FROM milestone_task mt " +
            "LEFT JOIN sys_user u ON mt.assignee_id = u.id " +
            "WHERE mt.project_id = #{projectId} AND mt.deleted = 0 " +
            "ORDER BY mt.milestone_id, mt.sort_order")
    @Results({
            @Result(property = "assigneeName", column = "assignee_name"),
            @Result(property = "assigneeAvatar", column = "assignee_avatar")
    })
    List<MilestoneTask> selectByProjectId(@Param("projectId") Long projectId);
}